...
for arch in archetypes:
    ...
    for hz in range(m):
        ...
        for r in range(len(df)):
            ...
            # if flnmV: # note: RSN1078 flatfile contains Z filename that
            # is not available for download. It has 'XXX' in its name.
            if flnmV and 'XXX' not in flnmV:
                ag_z = import_PEER(input_dir, flnmV)
            else:
                dt_x = ag_x[1, 0]-ag_x[0, 0]
                ag_z = np.column_stack((
                    np.linspace(0.00, dt_x*(len(ag_x)-1), len(ag_x)),
            ...
            # trim zero regions
            ag_max = np.max(np.column_stack((np.abs(ag_x), np.abs(ag_y))), axis=1)

            peak3 = np.sort(ag_max)[-3]
            idx_last = int(np.argwhere(ag_max > peak3/10)[-1])
            idx_first = int(np.argwhere(ag_max > peak3/100)[0])

            # import matplotlib.pyplot as plt
            # plt.plot(t_new[:idx_first], ag_x[:idx_first], 'blue')
            # plt.plot(t_new[idx_first:idx_last], ag_x[idx_first:idx_last], 'k')
            # plt.plot(t_new[idx_last:], ag_x[idx_last:], 'red')
            # # plt.plot(t_new, ag_max, 'red')
            # plt.show()

            ag_x = ag_x[idx_first:idx_last]
            ag_y = ag_y[idx_first:idx_last]
            ag_z = ag_z[idx_first:idx_last]
            t_max = len(ag_x)*0.005
